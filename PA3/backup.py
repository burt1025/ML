...
def binary_train(X, y, loss="perceptron", w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    if loss == "perceptron":
        ...
        for i in range(max_iterations):
            err = (w.dot(X.T) + b) - 2 * y + 1
            w_gradient = err.dot(X) / N
            b_gradient = np.sum(err) / N
        ...
        for t in range(max_iterations):
            ...
            for i in range(N):
                gradient = gradient + sigmoid(-(2*y[i]-1)*np.sum(w1*X1[i]))*(2*y[i]-1)*X1[i]
            w1 = w1 + step_size*gradient/float(N)
