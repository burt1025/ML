...
def get_k_means_plus_plus_center_indices(n, n_cluster, x, generator=np.random):
    ...
    while len(centers) < n_cluster:
        # Update min distance to centroids
...
class KMeans():
    ...
    def fit(self, x, centroid_func=get_lloyd_k_means):
        ...
        y = np.zeros(N)
        d = np.sum([np.sum((x[y == i] - centroids[i])) for i in range(self.n_cluster)]) / N

        iter = 0
        while iter < self.max_iter:
            l2 = np.sum(((x - np.expand_dims(centroids, axis=1))**2), axis=2)
            y = np.argmin(l2, axis=0)
            d_new = np.sum([np.sum((x[y == i] - centroids[i])) for i in range(self.n_cluster)]) / N
            if abs(d - d_new) <= self.e:
                break
            d = d_new
            centroids_new = np.array([np.mean(x[y == i], axis=0) for i in range(self.n_cluster)])

            idx = np.where(~np.isnan(centroids_new))
            centroids[idx] = centroids_new[idx]
            iter += 1
...
class KMeansClassifier():
    ...
    def predict(self, x):
        ...
        ##########################################################################
        r = np.argmin(np.sum(((x - np.expand_dims(self.centroids,axis=1))**2), axis=2), axis=0)
...
def transform_image(image, code_vectors):
    ...
    ##############################################################################
    N, M, C = image.shape
    dist = (image.reshape(N * M, C) - np.expand_dims(code_vectors, axis=1)) ** 2
    dist_sum = np.sum(dist, axis=2)
    r = np.argmin(dist_sum, axis=0)
