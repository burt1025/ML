...
class KNN:
    ...
    def get_k_neighbors(self, point):
        ...
        :return:  List[int]
        """
        dist_idx = zip([self.distance_function(x, point) for x in self.features], self.labels)
        sorted_dist = sorted(dist_idx, key = lambda t: (t[0], t[1]))
        labels = [int(idx) for dist, idx in sorted_dist]
        return labels[0:self.k]
    ...
    def predict(self, features):
        ...
        for data in features:
            neighbor_labels = self.get_k_neighbors(data)
            votes = {}
            for l in neighbor_labels:
                if l not in votes:
                    ...
                    votes[l] += 1
            predicted_labels.append(max(votes, key=votes.get))
